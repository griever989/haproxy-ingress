...
func createConfig(options options, resolver hatypes.Resolver) *config {
	...
	return &config{
		...
		backends:    hatypes.CreateBackends(options.shardCount, resolver),
...
func (c *config) Clear() {
	config := createConfig(c.options, c.resolver)
	*c = *config
}
